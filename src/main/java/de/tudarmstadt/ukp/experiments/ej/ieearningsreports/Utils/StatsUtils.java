package de.tudarmstadt.ukp.experiments.ej.ieearningsreports.Utils;
...
public class StatsUtils
{
	...
	/**
	 * Calculates the most frequent class baseline, based on the frequency of the most
	 * frequent class in the entire dataset.
	 * Note that the TRUE MFC baseline will vary due to data training/testing distribution.
	 * @param matrix
	 * @return the approximate MFC baseline
	 */
	public static Double getApproxMostFreqClassBaseline(int[][] matrix){
		int currentMostFreqClass = 0;
		int totalNumberInstances = 0;
		for (int i=0; i<matrix.length; i++){
			int classCount = 0;
			for(int j=0; j<matrix[i].length; j++){
				classCount = classCount + matrix[i][j];
			}
			totalNumberInstances = totalNumberInstances + classCount;
			if (classCount > currentMostFreqClass){
				currentMostFreqClass = classCount;
			}
		}
		return new Double(currentMostFreqClass) / totalNumberInstances;
	}
